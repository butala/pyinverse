...
*3-D FBP with the current \(\sqrt{f_u^2+f_v^2}\) filter saturates* ---
...
factor* \(|\cos\theta|\) (harmless; it just rescales the reconstruction)
for any \(\theta\neq 90^\circ\).
...
exact \(\theta=0\) case. This branch is not exercised by the validation
harness and is flagged as untested; it deserves its own study.
...
slice-by-slice equivalence check, and writes figures to =tests/out/=.
